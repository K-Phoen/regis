...
class DisableFlightModeTest extends TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->handler = new CommandHandler\Repository\DisableFlightMode($this->repositoriesRepo);
    }
    ...
    public function testItDisablesTheInspection()
    {
        ...
        $command = new Command\Repository\DisableFlightMode($repo);

        $this->repositoriesRepo->expects($this->once())
            ->method('save')
            ->with($this->callback(function (Entity\Repository $repo) {
                $this->assertFalse($repo->isFlightModeEnabled());
